...
string GetPascalCase(string  inStr)
{
	if ( string.IsNullOrEmpty(inStr))
	{
		return string.Empty;
	}		 
	string splitOn = @"(?i)[\s.,;_]+";
	string[] wordArray = Regex.Split(inStr, splitOn);	
	var newWord=string.Empty;	
	foreach (var word in wordArray)
	{
		newWord+=GetUppercaseFirst(GetLowerCase(word));
	}		
	return newWord;
}

var str="00000000000230_FIELD_INVESTIGATION_20181201";
 
GetPascalCase(str).Dump();
