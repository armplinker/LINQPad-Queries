<Query Kind="Statements">
 ...
 ReadOnlyCollection<string> HierarchicalSubDocTypes   = new ReadOnlyCollection<string>(
				  new string[]
				  {
						"KDOTBLP_FRACTURE_CRITICAL_INSP_DOC_GENERAL",  "KDOTBLP_FRACTURE_CRITICAL_INSP_REPORTS"  
				  }
				  );
				  
		
	 HierarchicalSubDocTypes.Dump();
	 var keyword = "KDOTBLP_FRACTURE_CRITICAL_INSP_DOC_GENERAL";
	 var docTypeLabel = "Fracture Critical";
     var documentTypeHomeFolder = "/FC/";
	 var documentsubTypeFolder = string.Concat(documentTypeHomeFolder ,"Reports");


	 var found = (from t in HierarchicalSubDocTypes where t.IndexOf(keyword) >=0 select t).Any();
	 found.Dump();

dynamic json = new System.Dynamic.ExpandoObject();
json.DocTypeKeyLabel = docTypeLabel;
json.DocSubTypeKeyLabel = "Reports";
json.DocTypeHomeFolder = documentTypeHomeFolder;
json.DocSubTypeFolder = documentsubTypeFolder;
...
serialized = Newtonsoft.Json.JsonConvert.SerializeObject(json);
serialized.Dump();

deSerialized = Newtonsoft.Json.JsonConvert.DeserializeObject(serialized);
deSerialized.Dump();
